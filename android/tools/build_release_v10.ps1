...
if ($env:DV_KEY_ALIAS -eq ('android' + 'debugkey')) { throw 'Debug signing aliases are forbidden for release builds.' }
...
& $gradle --no-daemon --console=plain clean assembleRelease bundleRelease
...
$out = Join-Path $project 'release-v10'
New-Item -ItemType Directory -Force -Path $out | Out-Null
$apk = Join-Path $out 'Dastyar-Vakil-v10.0.0-release.apk'
...
$unsignedBundle = Join-Path $project 'app\build\outputs\bundle\release\app-release.aab'
if (-not (Test-Path -LiteralPath $unsignedBundle -PathType Leaf)) { throw 'Gradle did not produce the release Android App Bundle.' }
$bundle = Join-Path $out 'Dastyar-Vakil-v10.0.0-release.aab'
Copy-Item -LiteralPath $unsignedBundle -Destination $bundle -Force
$jarsigner = Join-Path $jdk 'bin\jarsigner.exe'
& $jarsigner -keystore $keystore -storetype PKCS12 -storepass:env DV_KEYSTORE_PASSWORD -keypass:env DV_KEY_PASSWORD -sigalg SHA256withRSA -digestalg SHA-256 $bundle $env:DV_KEY_ALIAS
if ($LASTEXITCODE -ne 0) { throw 'AAB signing failed.' }
& $jarsigner -verify -certs $bundle | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'AAB signature verification failed.' }
Get-Item -LiteralPath $apk,$bundle | Select-Object FullName,Length,LastWriteTime
